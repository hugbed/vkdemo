...
private:
    ...
    // -------------------------- //
    void createInstance() {
        if (enableValidationLayers && false == checkValidationLayerSupport()) {
        ...
        for (const auto& validationLayer : validationLayers) {
            ...
            auto res = std::find_if(availableLayers.begin(), availableLayers.end(),
                [&validationLayer] (const auto& availableLayer) {
                    return strcmp(availableLayer.layerName, validationLayer);
                });
   ...
   static VkResult CreateDebugUtilsMessengerEXT(
    ...
    void pickPhysicalDevice() {
        ...
        if (count == 0) {
            throw new std::runtime_error("failed to find GPU for Vulkan support");
        }

        std::vector<VkPhysicalDevice> devices(count);
        vkEnumeratePhysicalDevices(m_instance, &count, devices.data());

        auto res = std::find_if(devices.begin(), devices.end(), [this] (const auto& device) {
    ...
    bool checkDeviceExtensionSupport(VkPhysicalDevice device) {
        ...
        std::set<std::string> requiredProperties(deviceExtensions.begin(), deviceExtensions.end());
        for (const auto& extensionProperty : extensionProperties) {
            requiredProperties.erase(extensionProperty.extensionName);
        }

        return requiredProperties.empty();
    }
    ...
    void createLogicalDevice()
    {
        ...
        // fetch the first queue since we have only one
        uint32_t graphicsQueueIndex = 0;
        vkGetDeviceQueue(m_device, indices.graphicsFamily.value(), graphicsQueueIndex, &m_graphicsQueue);
        vkGetDeviceQueue(m_device, indices.presentFamily.value(), graphicsQueueIndex, &m_presentQueue);
    }
    ...
    void createSwapChain() {
        ...
        // Query swapchain images, format, extent
    ...
    void createGraphicsPipeline() {
        ...
        // can use pipeline cache here to speed up cache creation process
    ...
    void drawFrame() {
        ...
        // submit result back to the swapchain for presentation
